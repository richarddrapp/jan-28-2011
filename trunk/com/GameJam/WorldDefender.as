 package com.GameJam{
	...
	public class WorldDefender extends ReddObject {				
		...
		var LaunchSpeed:Number = 8;
		
		var LaunchTimer:Timer;
		var SpawnTimer:Timer;
		var canLaunch:Boolean = true;
		
		public function WorldDefender() {				
			super(); //density = 0.7								
		...
		override public function Update(e:Event) :void  {					
			...
			if (key.isDown(keySpace))
			{
				if (canLaunch)
				{
					...
					var mis:Projectile = new Projectile(newX, newY, 10);
		...
		public function spawnMeteor(e:TimerEvent):void {	
			...
			var met:Meteor;
			if(Math.random() > 0.5)
				met = new Meteor(0, Math.random()*400, 30);
			else
				met = new Meteor(600, Math.random()*400, 30);
				
			atkAngle = Math.atan((met.y - this.y) / (met.x - this.x))
			metVX = (10) * Math.cos(atkAngle);
			metVY = (10) * Math.sin(atkAngle);
			if (met.x > 500)
			{
				metVX *= -1;
				metVY *= -1;
